...
class CONCERT(nn.Module):
    ...
    def counterfactualPrediction_uncertainty(self, X, sample_index, cell_atts,
                                 perturb_cell_id = None,
                                 target_cell_tissue = None,
                                 target_cell_perturbation = None,
                                 n_samples=1, batch_size=512
        ...
        """
        CaluUncertainty of Counterfactual Prediction
        ...
        for batch_idx in range(num_batch):
            ...
            mean_samples_ = torch.stack(mean_samples_, dim=0)
            mean_samples__ = torch.mean(mean_samples_, dim=0)
            sd_samples__ = torch.std(mean_samples_, dim=0)
            mean_samples.append(mean_samples__.data.cpu().detach())
            sd_samples.append(sd_samples__.data.cpu().detach())
            var_samples.append(torch.sqrt(p_v).data.cpu().detach())

        mean_samples = torch.cat(mean_samples, dim=0)
        sd_samples = torch.cat(sd_samples, dim=0)
        var_samples = torch.cat(var_samples, dim=0)
        #cal confidence interval for each cell
        lower_bound = mean_samples - 2.58*sd_samples
        upper_bound = mean_samples + 2.58*sd_samples
        #only output CI for perturbed cells
        mean_samples = mean_samples[perturb_cell_id]
        lower_bound = lower_bound[perturb_cell_id]
        upper_bound = upper_bound[perturb_cell_id]
        var_samples = var_samples
        return mean_samples.numpy(), lower_bound.numpy(), upper_bound.numpy(), var_samples.numpy()
